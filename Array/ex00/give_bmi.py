...
def give_bmi(height: list[int | float], weight: list[int | float])\
    ...
    of BMI values
    it will return error if list of height and  weight are not has\
          in same size, and if there is a value negatif or zero
    """
    try:
        ...
        if not all(isinstance(h, (int, float)) and h > 0 for h in height):
            raise ValueError("All height values must be positive integers \
                            or floats.")
...
def apply_limit(bmi: list[int | float], limit: int) -> list[bool]:
    ...
    a limit as parameters. It returns a list of booleans (True if \
        above the limit)
    """
    try:
        if not bmi:
        ...
        print(f"An inexpected error occured: {e}")
        return []
